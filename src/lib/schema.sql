...
-- Table for Webhook Audit Logs and Idempotency Tracking
...
DROP POLICY IF EXISTS "Authenticated Select for webhook_events" ON public.webhook_events;
CREATE POLICY "Authenticated Select for webhook_events" ON public.webhook_events
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated Insert for webhook_events" ON public.webhook_events;
CREATE POLICY "Authenticated Insert for webhook_events" ON public.webhook_events
  FOR INSERT TO authenticated WITH CHECK (true);

-- Table for Midtrans Clearing Account Transactions (COA 1200)
CREATE TABLE IF NOT EXISTS public.midtrans_clearing_transactions (
  ...
  clearing_status TEXT DEFAULT 'pending',
  property_id BIGINT,
  tenant_name TEXT,
...
DROP POLICY IF EXISTS "Authenticated All for midtrans_clearing_transactions" ON public.midtrans_clearing_transactions;
CREATE POLICY "Authenticated All for midtrans_clearing_transactions" ON public.midtrans_clearing_transactions
  FOR ALL USING (true) WITH CHECK (true);

-- Table for Bank Reconciliation Matches (COA 1010 vs 1200)
CREATE TABLE IF NOT EXISTS public.bank_reconciliation_matches (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  bank_statement_id BIGINT,
  clearing_transaction_id BIGINT,
  ...
  notes TEXT,
  created_by TEXT DEFAULT 'Finance Administrator',
  property_id BIGINT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.nearby_amenities (
  ...
  name TEXT NOT NULL,
  category TEXT NOT NULL CHECK (category IN ('transit', 'education', 'healthcare', 'shopping', 'dining', 'worship', 'lifestyle')),
...
-- Table for Contract Extensions (Realtime Tenant Lease Extensions)
CREATE TABLE IF NOT EXISTS public.contract_extensions (
  ...
  old_start_date DATE,
  old_duration_months INTEGER DEFAULT 1,
  extension_months INTEGER NOT NULL DEFAULT 1,
  monthly_rate NUMERIC(15,2) NOT NULL DEFAULT 0,
  total_amount NUMERIC(15,2) NOT NULL DEFAULT 0,
  payment_method TEXT NOT NULL DEFAULT 'Tunai / Transfer Manual',
...
DROP POLICY IF EXISTS "Public select for contract_extensions" ON public.contract_extensions;
CREATE POLICY "Public select for contract_extensions" ON public.contract_extensions
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Admin and Service Role all for contract_extensions" ON public.contract_extensions;
CREATE POLICY "Admin and Service Role all for contract_extensions" ON public.contract_extensions
  FOR ALL USING (true) WITH CHECK (true);

-- Enable Supabase Realtime CDC publication for contract_extensions
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND tablename = 'contract_extensions'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.contract_extensions;
  END IF;
EXCEPTION WHEN OTHERS THEN
  NULL;
END $$;
