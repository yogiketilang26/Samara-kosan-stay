-- Migration: 019_secure_contract_extensions.sql
-- Description: Menutup celah keamanan pada tabel contract_extensions (migration 018
-- mematikan RLS dan memberi akses penuh ke anon) dan memastikan RPC
-- settle_contract_extension() ikut memposting jurnal akuntansi secara atomic.

-- 1. Re-enable RLS
ALTER TABLE contract_extensions ENABLE ROW LEVEL SECURITY;

-- 2. Cabut akses berlebihan
REVOKE ALL ON contract_extensions FROM anon;
REVOKE ALL ON contract_extensions FROM authenticated;

-- 3. Policy: hanya authenticated yang boleh baca (untuk dashboard admin)
DROP POLICY IF EXISTS "Authenticated read contract_extensions" ON contract_extensions;
CREATE POLICY "Authenticated read contract_extensions" ON contract_extensions
  FOR SELECT TO authenticated
  USING (true);

-- 4. Grant select ke authenticated & all ke service_role
GRANT SELECT ON contract_extensions TO authenticated;
GRANT ALL ON contract_extensions TO service_role;

-- 5. Restrict RPC settle_contract_extension ke service_role saja
REVOKE EXECUTE ON FUNCTION settle_contract_extension(
  BIGINT, INT, NUMERIC, VARCHAR, VARCHAR, VARCHAR, TEXT
) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION settle_contract_extension(
  BIGINT, INT, NUMERIC, VARCHAR, VARCHAR, VARCHAR, TEXT
) TO service_role;

-- 6. Re-create settle_contract_extension() to include double-entry accounting posting
...
BEGIN
  ...
  -- 6. Post double-entry accounting journal (Kas & Bank / Pendapatan Sewa Kamar)
  BEGIN
